...
{
	int32_t ipart = qipart(n);
	int32_t fpart = qfpart(n);
	int32_t intdigits = 0;
	...
	for (int i = 0; i < FIXED_DECIMALDIGITS - 1; i++)
	{
		v = (v << 1) + (v << 3);
	}

	for (int i = 0; i < FIXED_FRACBITS; i++)
	{
		if (fpart & t)
		{
			st += v;
	...
	for (int i = 0; i < FIXED_DECIMALDIGITS; i++)
	{
		int j = offset - i;
		int d = st % 10;
		st = st / 10;
